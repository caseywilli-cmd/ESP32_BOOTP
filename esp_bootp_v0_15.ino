...
#include <WiFiClientSecure.h> // TLS transport for talking to GitHub over HTTPS
#include <HTTPClient.h>       // Used for both the release-info request and the .bin download
#include <Update.h>           // Flashes a downloaded .bin to the inactive OTA partition
#include <ArduinoJson.h>      // Parses the GitHub Releases API response (Library Manager: "ArduinoJson" by Benoit Blanchon, v7+)
...
// Firmware version + OTA update source.
// Bump FIRMWARE_VERSION every time you cut a new GitHub release so the
// dashboard's "Check for Updates" button can tell a new release apart from
// what's already running (see fetchLatestRelease() below for how the
// comparison works). GITHUB_OWNER/GITHUB_REPO must point at the repo whose
// Releases page hosts the compiled .bin files.
#define FIRMWARE_VERSION "0.15"
const char* GITHUB_OWNER = "caseywilli-cmd"; // TODO: set this to your repo's owner
const char* GITHUB_REPO  = "ESP32_BOOTP";        // TODO: set this to your repo's name
...
// ==========================================
// The dashboard's "Check for Updates" / "Install Update" buttons (see
// /api/ota_check and /api/ota_install below) both talk to GitHub over
// HTTPS, which is only reachable via the Wi-Fi hotspot -- the Ethernet
// side is an isolated field network with no internet route, and it's set
// as the default outbound route once it comes up (see the ETH_CONNECTED
// handler in setup()). The two functions below temporarily swap the
// default route to Wi-Fi for the duration of a GitHub request and then
// restore it, so a check/install can't accidentally leave BOOTP/DHCP-
// adjacent traffic trying to go out over the wrong interface afterward.

// Makes Wi-Fi the default outbound route. Call this immediately before any
// GitHub request, and always pair it with pinRouteToEthernet() afterward
// (including on early-return error paths) so the swap doesn't outlive the
// request that needed it.
void pinRouteToWiFi() {
    esp_netif_t* wifiNetif = esp_netif_get_handle_from_ifkey("WIFI_STA_DEF");
    if (wifiNetif) esp_netif_set_default_netif(wifiNetif);
}

// Restores Ethernet as the default outbound route. Mirrors the routing
// set up when the Ethernet link first comes up in setup().
void pinRouteToEthernet() {
    esp_netif_t* ethNetif = ETH.netif();
    if (ethNetif) esp_netif_set_default_netif(ethNetif);
}

// Queries the GitHub Releases API for the newest release of
// GITHUB_OWNER/GITHUB_REPO and returns its version tag and the direct
// download URL of its .bin asset. Shared by both /api/ota_check (which
// only needs the version, to decide whether to show "update available")
// and /api/ota_install (which needs the URL too, to actually download it)
// so the GitHub-talking logic only exists in one place.
//
// Returns true on success with outVersion/outAssetUrl filled in; on
// failure returns false with `error` set to something safe to show a
// technician directly in the dashboard (no internal details/stack traces).
bool fetchLatestRelease(String &outVersion, String &outAssetUrl, String &error) {
    pinRouteToWiFi();

    WiFiClientSecure client;
    // GitHub's certificate chain rotates over time, and the actual asset
    // download redirects to a second host (objects.githubusercontent.com)
    // with its own chain -- pinning a specific root CA for both would be
    // fragile and would silently break updates whenever either cert
    // rotates. Skipping validation is a deliberate tradeoff for this
    // internal tool (the request carries no credentials or sensitive
    // data); pin a CA bundle instead if that tradeoff isn't acceptable.
    client.setInsecure();

    HTTPClient http;
    String url = "https://api.github.com/repos/" + String(GITHUB_OWNER) + "/" + String(GITHUB_REPO) + "/releases/latest";
    http.begin(client, url);
    // GitHub's API normally replies with Transfer-Encoding: chunked (no
    // Content-Length) rather than a fixed-size body. Reading straight from
    // http.getStream() bypasses HTTPClient's chunk-decoding entirely, so
    // ArduinoJson ends up parsing raw chunk-size markers along with the
    // JSON -- which is exactly what "Couldn't parse GitHub's response"
    // was: it usually parses fine and then intermittently fails depending
    // on how GitHub happens to chunk that particular response. Forcing
    // HTTP/1.0 asks the server not to use chunked encoding in the first
    // place (see https://arduinojson.org/v6/how-to/use-arduinojson-with-httpclient/).
    http.useHTTP10(true);
    http.addHeader("User-Agent", "esp32-bootp-tool"); // GitHub's API 403s any request with no User-Agent
    http.addHeader("Accept", "application/vnd.github+json");

    int httpCode = http.GET();
    if (httpCode != HTTP_CODE_OK) {
        error = "Couldn't reach GitHub (HTTP " + String(httpCode) + "). Check that the hotspot has internet access.";
        http.end();
        pinRouteToEthernet();
        return false;
    }

    // Belt-and-suspenders alongside useHTTP10 above: read the full response
    // into a String via getString() (which goes through HTTPClient's own
    // chunk-decoding logic) rather than handing ArduinoJson the raw stream.
    // A release's JSON body only runs a few KB, so buffering it fully here
    // is cheap -- and it sidesteps the whole class of chunked-encoding
    // parsing bugs, several of which people still hit with useHTTP10 alone
    // when combined with ArduinoJson's Filter option.
    String body = http.getString();
    http.end();
    pinRouteToEthernet();

    // A release's full JSON body (changelog text, uploader info, etc.) can
    // run to several KB -- filtering keeps everything except the two
    // fields we actually need off the JsonDocument's memory, rather than
    // storing (and then discarding) all of it. See the ArduinoJson
    // filtering docs for the "index 0 stands for every array element"
    // convention used for the assets list below.
    JsonDocument filter;
    ...
    JsonDocument doc;
    DeserializationError jsonErr = deserializeJson(doc, body, DeserializationOption::Filter(filter));

    if (jsonErr) {
        // Logged (not just shown in the dashboard) since the dashboard
        // message stays generic on purpose -- if this ever comes back,
        // the Serial Monitor (38400 baud) will have the specific reason
        // (e.g. "InvalidInput", "NoMemory", "EmptyInput").
        Serial.printf("GitHub JSON parse failed: %s\n", jsonErr.c_str());
        error = "Couldn't parse GitHub's response.";
        return false;
    }
    ...
    // Releases are typically tagged like "v0.15" -- strip a leading 'v' so
    // this compares cleanly against FIRMWARE_VERSION ("0.15").
    ...
    if (outAssetUrl == "") {
        error = "Latest release (" + outVersion + ") has no .bin file attached.";
        return false;
    }
    return true;
}
...
void setup() {
    ...
    // Settings" card, plus the running firmware version (used both by the
    // "Firmware" card and, after an OTA install, by the dashboard's
    // reboot-detection polling -- see waitForReboot() in webpage.h).
    server.on("/api/settings", HTTP_GET, [](AsyncWebServerRequest *request){
        String json = "{\"ip\":\"" + current_eth_ip + "\",\"sn\":\"" + current_eth_sn + "\",\"version\":\"" + FIRMWARE_VERSION + "\"}";
    ...
    // Reports whether a newer firmware release exists on GitHub, without
    // downloading or installing anything. See fetchLatestRelease() for how
    // "newer" is determined and why this has to run over Wi-Fi.
    server.on("/api/ota_check", HTTP_GET, [](AsyncWebServerRequest *request){
        String version, assetUrl, error;
        if (!fetchLatestRelease(version, assetUrl, error)) {
            request->send(200, "application/json", "{\"error\":\"" + error + "\"}");
            return;
        }
        bool updateAvailable = (version != String(FIRMWARE_VERSION));
        String json = "{\"current\":\"" + String(FIRMWARE_VERSION) + "\",\"latest\":\"" + version +
                      "\",\"updateAvailable\":" + (updateAvailable ? "true" : "false") + "}";
        request->send(200, "application/json", json);
    });

    // Downloads the newest release's .bin from GitHub and flashes it to the
    // inactive OTA partition. This re-checks GitHub itself (rather than
    // trusting whatever /api/ota_check last saw) so a check that happened
    // a while ago can't install a stale/wrong asset URL.
    //
    // IMPORTANT: this requires the sketch to be built with an OTA-capable
    // partition scheme (Arduino IDE: Tools > Partition Scheme > anything
    // with two "app" slots, e.g. "Default 4MB with spiffs" or "Minimal
    // SPIFFS"). With a no-OTA scheme, Update.begin() below will fail with
    // a "not enough space" style error.
    server.on("/api/ota_install", HTTP_GET, [](AsyncWebServerRequest *request){
        String version, assetUrl, error;
        if (!fetchLatestRelease(version, assetUrl, error)) {
            request->send(200, "application/json", "{\"error\":\"" + error + "\"}");
            return;
        ...
        pinRouteToWiFi();
        WiFiClientSecure client;
        client.setInsecure(); // see the note on this in fetchLatestRelease()
        HTTPClient http;
        http.begin(client, assetUrl);
        // GitHub release asset URLs 302-redirect to a separate download
        // host (objects.githubusercontent.com); FORCE (rather than STRICT)
        // follows that redirect reliably -- STRICT has been reported to
        // fail on exactly this kind of GitHub redirect.
        http.setFollowRedirects(HTTPC_FORCE_FOLLOW_REDIRECTS);
        http.addHeader("User-Agent", "esp32-bootp-tool");

        int httpCode = http.GET();
        if (httpCode != HTTP_CODE_OK) {
            http.end();
            pinRouteToEthernet();
            request->send(200, "application/json", "{\"error\":\"Firmware download failed (HTTP " + String(httpCode) + ")\"}");
            return;
        }
        ...
        if (len <= 0) {
            http.end();
            pinRouteToEthernet();
            request->send(200, "application/json", "{\"error\":\"Firmware download had no content length\"}");
            return;
        }
        ...
        if (!Update.begin(len)) {
            http.end();
            pinRouteToEthernet();
            request->send(200, "application/json", "{\"error\":\"Not enough free space for the update: " + String(Update.errorString()) + "\"}");
            return;
        }

        // Streams the download straight into flash rather than buffering
        // the whole .bin in RAM first -- this is the standard ESP32 OTA-
        // over-HTTP idiom.
        ...
        if (written != (size_t)len) {
            Update.abort();
            request->send(200, "application/json", "{\"error\":\"Download incomplete (" + String(written) + "/" + String(len) + " bytes)\"}");
            return;
        }

        if (!Update.end()) {
            request->send(200, "application/json", "{\"error\":\"Flashing failed: " + String(Update.errorString()) + "\"}");
            return;
        }

        // Success -- send the response now, then reboot shortly after
        // (same deferred-reboot pattern as /api/reboot and
        // /api/set_network) so this reply actually reaches the browser
        // before the device restarts into the new firmware.
        request->send(200, "application/json", "{\"success\":true,\"version\":\"" + version + "\"}");
        shouldReboot = true;
        rebootTimer = millis();
    });
