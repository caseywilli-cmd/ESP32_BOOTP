...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .ota-result { font-size: .85rem; }
    .ota-ok { color: var(--success); font-weight: 600; }
    .ota-fail { color: var(--danger); font-weight: 600; }
    .ota-update { color: var(--primary); font-weight: 600; }
...
<body>
  ...
  <div class="container">
    ...
    <div class="card">
      <h3>Firmware</h3>
      <div class="current-ip">Running version: <span id="fw_version">Loading...</span></div>
      <div class="ota-row">
        <button id="otaCheckBtn" onclick="checkForUpdates()">Check for Updates</button>
        <button id="otaInstallBtn" class="danger" style="display:none" onclick="installUpdate()">Install Update</button>
        <span id="otaResult" class="ota-result"></span>
      </div>
    </div>
    ...
    <div class="footer-note">Firmware <span id="footerVersion">v0.14</span></div>
  </div>
  ...
  <script>
    ...
    function loadSettings() {
      fetch('/api/settings').then(r => r.json()).then(data => {
        ...
        document.getElementById('footerVersion').innerText = 'v' + data.version;
    ...
    // Asks the tool to check GitHub for a newer release without installing
    // anything yet. Shows an "Install Update" button only if one is found.
    function checkForUpdates() {
      let checkBtn = document.getElementById('otaCheckBtn');
      let installBtn = document.getElementById('otaInstallBtn');
      let result = document.getElementById('otaResult');
      checkBtn.disabled = true;
      installBtn.style.display = 'none';
      result.className = 'ota-result';
      result.innerText = 'Checking GitHub for the latest release...';
      ...
      fetch('/api/ota_check')
        ...
        .then(data => {
          ...
          if (data.error) {
            result.className = 'ota-result ota-fail';
            result.innerText = data.error;
            return;
          }
          if (data.updateAvailable) {
            result.className = 'ota-result ota-update';
            result.innerText = `Update available: v${data.latest} (currently v${data.current})`;
            installBtn.dataset.version = data.latest;
            installBtn.style.display = 'inline-block';
          } else {
            result.className = 'ota-result ota-ok';
            result.innerText = `Up to date (v${data.current})`;
          }
        })
        .catch(() => {
          checkBtn.disabled = false;
          result.className = 'ota-result ota-fail';
          result.innerText = 'Could not reach GitHub. Check that the hotspot has internet access.';
        });
    }

    // Downloads and flashes the release that checkForUpdates() found, then
    // waits for the tool to come back up on the new version.
    function installUpdate() {
      let installBtn = document.getElementById('otaInstallBtn');
      let checkBtn = document.getElementById('otaCheckBtn');
      let result = document.getElementById('otaResult');
      let targetVersion = installBtn.dataset.version;

      if (!confirm(`Install v${targetVersion} now? The tool will be unreachable for a minute or two while it downloads, flashes, and reboots. Do not power off the device during this time.`)) return;

      installBtn.disabled = true;
      checkBtn.disabled = true;
      result.className = 'ota-result';
      result.innerText = 'Downloading and installing update -- do not power off the device...';

      fetch('/api/ota_install')
        .then(r => r.json())
        .then(data => {
          if (data.error) {
            result.className = 'ota-result ota-fail';
          ...
          waitForReboot(targetVersion);
        })
        .catch(() => {
          // A successful install ends with the device rebooting, which
          // kills this request before a response can be sent back -- so a
          // network error here is actually the EXPECTED outcome of success,
          // not necessarily a failure. Wait for the tool to come back
          // rather than reporting a hard error immediately.
          waitForReboot(targetVersion);
        });
    }

    // Polls the tool until it comes back online reporting the version we
    // just installed. Checking the reported version (not just "did it
    // respond") matters here: the actual reboot is deferred by ~2s after a
    // successful install (see /api/ota_install), so the OLD firmware is
    // still briefly responsive right after installUpdate() gets its
    // success response -- polling for the new version specifically avoids
    // mistaking that brief window for the update having landed.
    function waitForReboot(targetVersion) {
      let result = document.getElementById('otaResult');
      result.innerText = 'Update sent -- waiting for the tool to reboot and come back online...';
      let attempts = 0;
        ...
        fetch('/api/settings')
          ...
          .then(data => {
            if (data.version === targetVersion) {
              clearInterval(poll);
              result.className = 'ota-result ota-ok';
              result.innerText = `Update installed -- now running v${data.version}. Refreshing...`;
              setTimeout(() => location.reload(), 1500);
            }
            // else: still the old version, keep polling
          })
          .catch(() => { /* expected while the device is mid-reboot */ });

        if (attempts > 40) { // ~2 minutes at 3s/poll
          clearInterval(poll);
          result.className = 'ota-result ota-fail';
          result.innerText = "Tool hasn't come back online yet -- check it directly before trying again.";
